...
**REST API:**
```bash
curl -X GET "https://api.solixdb.xyz/v1/transactions?protocol_name=jupiter_v6&date_from=2025-01-01&date_to=2025-01-31&limit=100"
```
...
**REST API:**
```bash
curl -X GET "https://api.solixdb.xyz/v1/transactions/5VERv8NMxzbPDohNpJhHopN2C9R4G5GaPevYf5hLJ8Z7"
```
...
**REST API:**
```bash
curl -X GET "https://api.solixdb.xyz/v1/analytics/protocols?protocol_name=jupiter_v6&date_from=2025-01-01"
```
...
**REST API:**
```bash
curl -X GET "https://api.solixdb.xyz/v1/analytics/time-series?protocol_name=jupiter_v6&date_from=2025-01-01&date_to=2025-01-31&granularity=hour"
```
...
**REST API:**
```bash
curl -X GET "https://api.solixdb.xyz/v1/analytics/fees?protocol_name=jupiter_v6&date_from=2025-01-01&date_to=2025-01-31"
```
...
async function getTransactions(protocol, dateFrom, dateTo) {
  ...
  const response = await fetch(
    `https://api.solixdb.xyz/v1/transactions?${params}`
  );
...
while (true) {
  const response = await fetch(
    `https://api.solixdb.xyz/v1/transactions?limit=${limit}&offset=${offset}`
  );
  const data = await response.json();
...
for (const protocol of protocols) {
  const response = await fetch(
    `https://api.solixdb.xyz/v1/analytics/protocols?protocol_name=${protocol}`
  );
...
```javascript
const response = await fetch(
  'https://api.solixdb.xyz/v1/transactions?protocol_name=jupiter_v6&limit=10000'
);
